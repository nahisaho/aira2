...
Type: functional
Pattern: ubiquitous
Statement: The system shall durably persist, across a process restart, all state required to preserve every existing requirement's pre-restart behavior, including at minimum: user accounts and roles, LLM provider credentials, per-user default backend/model selections, and project-level backend overrides (REQ-LLMBACKEND-002..005/008), project membership/shares/authorization state and the per-user audit log (REQ-MULTIUSER-002..006/009..012), team entities/team membership/team-based project shares and pending/cancelled/consumed invitations (REQ-MULTIUSER-013..017/025/026/029..031/039/045), each account's self-service profile/verified-email/pending-email-change state, password credential state, password-reset-token state including its per-email/per-source rate-limit log (REQ-MULTIUSER-036), and enrolled MFA (TOTP) factor state including its accepted-time-step replay-prevention record and consecutive-failure/lockout counter (REQ-MULTIUSER-018..021/027/028/032..034/036..038/040..044/049), every active session and each account's credential version (REQ-MULTIUSER-023/024/046..050), ELN records/protocols/versions/signatures/audit trails (REQ-ELN-*), per-project Agent Skills/MCP configuration (REQ-AGENTCONFIG-*) — all in a SQLite-backed store — and each project's Graph RAG indexed vector/graph data sufficient to answer a previously indexed query (REQ-GRAPHRAG-001..003/013..015), not merely index metadata, in its own durable file-backed store consistent with Graph RAG's per-project process/data-directory architecture (REQ-GRAPHRAG-*) rather than necessarily within the same SQLite database. Every stateful design component's persistence owner (its backing SQLite table(s) or, for Graph RAG, its own file-backed store) must be identified in its design entry and covered by a restart test; state is not exempt from this requirement merely because it was added after this requirement was first written.
Acceptance: A test starts the actual server process (per REQ-RUNTIME-001) against a file-backed database path, creates through its normal interfaces a user, a credential, a signed ELN record, a project share (producing an audit-log entry per REQ-MULTIUSER-006), an Agent Skills/MCP configuration, a Graph RAG-indexed document, a team with a team-based project share, a pending invitation, a second invitation that is then accepted (consumed) and a third that is cancelled, a self-service profile with a pending email change and a subsequently-confirmed verified email, a password-reset token, one exhausted password-reset rate-limit window for a distinct email, an enrolled and activated MFA (TOTP) factor with one already-accepted time-step and a triggered consecutive-failure lockout, and an active session; stops that server process and starts a fresh one against the same database path (a real process restart, not merely a fresh in-memory instance); and confirms all of the following hold after restart, through the same normal interfaces (REST API or component method, not direct database inspection): the user/credential/share/configuration are retrievable and the audit-log entry is intact; the ELN record's signature and audit trail are intact; a Graph RAG query against the pre-restart indexed document returns a correctly cited result without re-indexing; the team/team-share are retrievable, the pending invitation can still be accepted, the already-consumed invitation cannot be accepted again, and the cancelled invitation cannot be accepted; the verified email is intact and still governs invitation-matching/reset-eligibility; a new password-reset token can be issued and completed for the account, while the exhausted rate-limit window for the other email still rejects a new request; the MFA factor still requires a valid, not-yet-used TOTP code at login, rejects replay of the already-accepted time-step, and remains locked out until its cooldown elapses; and the active session remains valid (no forced re-login), lists with its correct display ID/creation/expiry, and carries its correct credential version.
