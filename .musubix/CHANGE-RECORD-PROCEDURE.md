...
> **musubix3 0.1.17 以降の更新（2026-09-13 反映）**: この問題は
> `nahisaho/musubix3#20` として報告し、v0.1.17 で修正済みです。
> `change-record` は現在、直前フェーズからフィンガープリントが変化して
> いない場合、記録前に **即座に** `CHANGE_REQUIREMENTS_UNCHANGED_AT_RECORD`
> 等のエラーで拒否します（append-only で永続化される前に検知できる）。
> 加えて `--dry-run`（実際に記録せず結果だけプレビュー）と
> `--allow-unchanged`（不具合修正など、意図的に変更なしで記録したい場合の
> 明示的な回避）が追加されました。本リポジトリは `package.json` の
> `devDependencies` に `musubix3@^0.1.17` を固定インストール済みで、
> `npx musubix3` は常にこのバージョンを解決します。
>
> ただし、このツール側の fail-fast は **安全網であって手順の代替ではない**。
> 以下の「都度実行」の運用は引き続き守ること。迷ったら先に
> `--dry-run` で確認してから本記録する。
